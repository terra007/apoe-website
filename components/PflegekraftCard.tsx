...
export default function PflegekraftCard({ pflegekraft: pk }: PflegekraftCardProps) {
  ...
  return (
    <Link
      ...
      className="group block rounded-2xl border border-navy-100 bg-white shadow-card hover:shadow-card-hover transition-all duration-200 overflow-hidden hover:-translate-y-0.5"
      aria-label={`Profil von ${pk.name} öffnen`}
    >
      {/* Video Thumbnail Preview */}
      <div
        className={`relative h-40 bg-gradient-to-br ${pk.avatarColor} flex items-center justify-center overflow-hidden`}
      >
        {/* Pattern */}
        <div
